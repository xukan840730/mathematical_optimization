...
%   $Revision: 1.14.4.7 $  $Date: 2009/10/16 06:42:58 $
    ...
    if ~err
        ...
        if ~err
            if ~isempty(findcstr(ud.labelList,label))
                ...
                str2display = sprintf('By importing "%s", you are replacing an already existing object in SPTool named "%s." Any objects that depend on "%s" will \nbe altered. \n\nAre you sure you want to import?',label,label,label);
                switch questdlg(...
                        str2display,...
