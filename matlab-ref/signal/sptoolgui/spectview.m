...
%         methodNameList = get(ud.hand.methodPopup,'string');
    ...
    case 'apply'
        ...
        sts = getSpecTitleStr(ud);
        set(get(ud.mainaxes,'title'),'String',xlate(sts));
...
if numel(ud.SPToolIndices)>1,
    ...
    switch lower(cm)
        case 'fft'
            sts = xlate('FFT Spectrum Estimate');
        case 'music'
            sts = sprintf('%s Pseudospectrum',cm);
        case 'mod. covar.'
            sts = xlate('Modified Covariance Power Spectral Density Estimate'); 
        case 'mtm'
            sts = xlate('Thompson Multitaper Power Spectral Density Estimate');
        case 'welch'
            sts = xlate('Welch Power Spectral Density Estimate');
        case 'yule ar'
            sts = xlate('Yule AR Power Spectral Density Estimate');
        case 'burg'
            sts = xlate('Burg Power Spectral Density Estimate');
        case 'covariance'
            sts = xlate('Covariance Power Spectral Density Estimate');
        otherwise
            % All other methods use the default string
            sts = sprintf('%s Power Spectral Density Estimate',cm); 
    end
