...
%
%   See also INTERP, DECIMATE, RESAMPLE and in the Filter Design Toolbox,
%   FIRHALFBAND, FIRNYQUIST, FIREQINT, MFILT.
...
%   $Revision: 1.6.4.6 $  $Date: 2009/05/23 08:13:36 $
...
%    R   discrete time sampling period (input to filter assumed 0 else)
    ...
    if ~(rem(n,2) || rem(R,2))
       warning(generatemsgid('InvalidParam'), ...
           'Linear phase filter not possible for both R and N even');
    end
...
%    pause;
    ...
    error(generatemsgid('InvalidParam'),'Sorry, this type of filter is not recognized')
end
