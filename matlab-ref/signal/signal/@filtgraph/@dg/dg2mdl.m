...
%   $Revision: 1.1.6.18 $  $Date: 2009/08/11 15:48:22 $
        ...
        for m = efflist
            ...
            switch curblock.blocktype
                ...
                case 'delay'
                    ...
                    if isfield(get_param(hblk,'DialogParameters'),'vinit')
                        set_param(hblk, 'Position',pos, 'ShowName','off',...
                            'Orientation',get(curblock,'orientation'),'vinit',IC);
                    else
                        % For backwards compatibility only. The Delay block
                        % from Signal Processing Blockset may have been
...
spblks_avail = isspblksinstalled;
    if spblks_avail,
        ...
        if isempty(find_system(0,'Name',dspblklibname))
            isdspblkloaded = 1;
            load_system(dspblklibname);
        end
        % load dspindex for multiport selector
        if isempty(find_system(0,'Name','dspindex'))
...
fiblklibname = 'fixpt_lib_4';
    if isempty(find_system(0,'Name',fiblklibname))
        isfiblkloaded = 1;
...
for ii = 1:length(coeffnames)
    ...
    [varsExist, existMsg] = chkIfVarExistInWksp(coeffnames{ii});
    if varsExist,  warning(generatemsgid('VariableExist'),existMsg); end
