...
%   $Revision: 1.14.4.5 $  $Date: 2009/05/23 08:13:44 $

error(nargchk(2,6,nargin,'struct'))

[z,p,k,direction_flag,scale,krzflag,msg] = parseinput(varargin{:});
...
%------------------------------------------------------------------------------------
function [z,p,k,direction_flag,scale,krzflag,msg] = parseinput(varargin)
...
[temp,ind] = sort(abs(p_conj - exp(j*angle(p_conj)))); %#ok
p_conj = p_conj(ind);
...
if krzflag && ~isempty(z_real) && ~isempty(find(z_real == -z_real(1))),
    while ~isempty(z_real) && ~isempty(find(z_real == -z_real(1))),
...
for i = 1:length(z_real),
    if ~isempty(p_conj),
        [temp,ind] = min(abs(z_real-p_conj(1))); %#ok
        new_z = [new_z; z_real(ind)];
        z_real(ind) = [];
        p_conj(1) = [];
    elseif ~isempty(p_real),
        [temp,ind] = min(abs(z_real-p_real(1))); %#ok
        new_z = [new_z; z_real(ind)];
        z_real(ind) = [];
        p_real(1) = [];
    else
