...
%   $Revision: 1.1.6.9 $  $Date: 2007/12/14 15:03:57 $
...
if (num_bands ~= floor(num_bands)),
  error(generatemsgid('MustBeEven'),'F must contain an even number of band edge entries.');
end
if any(diff(edges) <= 0),
   error(generatemsgid('InvalidFreqVec'),'Band edges must be monotonically increasing.')
end
...
for ii=1:length(varargin),
  ...
  elseif ~ischar(varargin{ii}),
     if ii==1,
       ...
       error(generatemsgid('MustBeAString'),'Expecting a string argument.');
     end
  else
    ...
    otherwise
      error(generatemsgid('InvalidParam'),['Invalid argument "' ppv '" specified.']);
    end
  end
end
...
if strcmp(h_sym,'unspecified'),
   ...
   if any(edges < 0),
     ...
     try
        ...
        warning(generatemsgid('NotSupported'),['Frequency response function does not support the '...
            ' ''default'' symmetry query.  Assuming symmetry=''none''']);
        h_sym = 'none';
     end
     if ~any(strcmp(h_sym,sym_opts)),
       error(generatemsgid('InvalidParam'),['Invalid default symmetry option "' h_sym '" returned ' ...
              'from response function "' func2str(filt_call) '".  Must be one ' ...
...
if strcmp(fdomain, 'whole'),
  ...
  if any(edges < -0.5 | edges > 0.5),
    error(generatemsgid('InvalidRange'),['Frequency band edges must be in the range [-1,+1] for ' ...
           'designs with SYM=''' h_sym '''.']);
  end
else
  % Domain is [0,+1] for user input, [0,.5] internally
  if any(edges < 0 | edges > 0.5),
    error(generatemsgid('InvalidRange'),['Frequency band edges must be in the range [0,+1] for ' ...
           'designs with SYM=''' h_sym '''.']);
  end
end
...
if strcmp(h_sym, 'odd')
    ...
    if any (abs(DES_CRMZ(sk)) > sqrt (eps))
        error(generatemsgid('SignalErr'),'Odd symmetric (antisymmetric) filters must have a zero gain at DC.');
    end
end
     
if strcmp(h_sym,'real') && all(edges >= 0)
   % We need to make DES and WT conjugate symmetric
   %error(generatemsgid('InvalidParam'),['Frequency band edges must be specified over the entire ' ...
...
if (max(GRID_CRMZ) > edges(end)) || (min(GRID_CRMZ) < edges(1)),
  error(generatemsgid('InternalError'),'Internal error: Grid frequencies out of range.')
end
...
if ~isequal(size(DES_CRMZ), size(GRID_CRMZ)) || ...
  ...
  str = ['Incorrect size of results from response function "' ...
         func2str(filt_call) '".  Sizes must be the same size as the ' ...
         'frequency grid GF.'];
  error(generatemsgid('InvalidDimensions'),str);
end
