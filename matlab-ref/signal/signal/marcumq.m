...
%   $Revision: 1.1.6.2 $  $Date: 2009/08/11 15:47:48 $
...
if ~isempty(msg)
    error('comm:marcumq:numInArg',msg);
end
...
if (any(~isreal(a)) || any(~isreal(b)) || any(~isreal(m)))
    error('comm:marcumq:nonrealArgs', 'Arguments must be real.');
end

if (any(a(:)<0) || any(b(:)<0))
    error('comm:marcumq:negativeArgs', ...
        'The first two arguments must be nonnegative.');
end
...
elseif (~isequal(size(a),size(b)))
    error('comm:marcumq:inconsistentDims', ...
...
if Y < NX + N  % Compute 1-P(N,X,Y) using form 4 or form 3
    if logCB > log(1e-2)
        Form = 4;   % Form 4 corresponds to Eq. 11
    else
        Form = 3;   % Form 3 corresponds to Eq. 10
    end
else           % Compute P(N,X,Y) using form 2 or form 1
    if logCB > log(1e-2)
        Form = 2;   % Form 2 corresponds to Eq. 9
    else
        Form = 1;   % Form 1 corresponds to Eq. 8
    end
end

switch Form
    case 1
        % Form 1 corresponds to Eq. 8
        ...
        % computed above, precompute start of inner summation.  If not,
        ...
        outerArg = expA(NX,k);
        outerSum = outerArg*innerSum;

        % Sum until the outerSum is no longer increasing
        while outerArg*innerSum>realmin
            m = m + 1;
            k = k + 1;
            innerArg = innerArg*Y/m;
            innerSum = innerSum + innerArg;
            outerArg = outerArg*NX/k;
            outerSum = outerSum + outerArg*innerSum;
        end
