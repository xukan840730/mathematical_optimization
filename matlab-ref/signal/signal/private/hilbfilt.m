...
%   $Revision: 1.7.4.5 $  $Date: 2007/12/14 15:15:10 $
...
if Le == 2,
  if any(F <= 0),
    error(generatemsgid('MustBePositive'),['Band edges must be strictly positive for ' ...
           'single-band Hilbert designs.']);
  end
elseif Le == 4,
  if F(2)*F(3) > 0,
    error(generatemsgid('InvalidParam'),['Transition band must include DC for two-band' ...
           'Hilbert designs.']);
  end
else
  error(generatemsgid('InvalidDimensions'),'There must be either 2 or 4 band edges for Hilbert designs.')
end
