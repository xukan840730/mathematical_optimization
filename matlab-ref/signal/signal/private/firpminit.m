...
%   $Revision: 1.1.6.4 $  $Date: 2009/05/23 08:16:19 $

error(nargchk(3,6,nargin,'struct'));

if all( ff(2:2:end)-ff(1:2:end) == 0),
	error(generatemsgid('InvalidFreqVec'),'At least one frequency band must have a non-zero width.')
end

if order < 3
    error(generatemsgid('InvalidRange'),'Filter order must be 3 or more.');
end
%
...
for i=1:length(varargin)
    if iscell(varargin{i})
        lgrid = varargin{i}{:};
        if lgrid < 16,
            warning(generatemsgid('InvalidParam'),'Grid density should be at least 16.');
        end
        if lgrid < 1,
            error(generatemsgid('MustBePositive'),'Grid density should be positive.');
        end
        varargin(i) = [];
        break
    end
...
%
if rem(length(ff),2)
    error(generatemsgid('MustBeEven'),'The number of frequency points must be even.');
end
if any((ff < 0) | (ff > 1))
    error(generatemsgid('InvalidRange'),'Frequencies must lie between 0 and 1.');
end
df = diff(ff);
if (any(df < 0))
...
if (any(sign(wtx) == 1) && any(sign(wtx) == -1)) || any(sign(wtx) == 0),
    error(generatemsgid('InvalidRange'),'All weights must be greater than zero.');
end
...
elseif iscell(aa)
    ...
    if ~isempty(msg2),
        ...
        warning(generatemsgid('OrderIncreased'),msg2);%#ok
    end
...
% [0,1].
[des,wt] = frf(order, ff, grid, wtx, frf_params{:}); %#ok
