...
%   $Revision: 1.1.6.10 $  $Date: 2009/08/29 08:28:53 $

% Title for the dialog
title = sprintf('Export To Workspace');
% Properties of the dialog (setting units to pixels to guard against
% someone setting defaultfigureunits to something else)
hDialog = dialog('Visible', 'off', 'Name', title, 'WindowStyle', 'normal', 'Unit', 'pixels');
...
checkboxLabels = {sprintf('Export problem and options to a MATLAB structure named:'); ...
                  sprintf('Include information needed to resume this run');...
                  sprintf('Export options to a MATLAB structure named:');...
                  sprintf('Export results to a MATLAB structure named:')};
...
function OKCallback(obj, eventdata, dialog, cb, e, optStruct, probStruct,resultStruct)
    ...
    if ~isempty(badnames)
        if (length(badnames) == 1)
            badnamemsg = sprintf('"%s" is not a valid MATLAB variable name.', ...
                badnames{1});
        elseif (length(badnames) == 2)
            badnamemsg = sprintf('"%s" and "%s" are not valid MATLAB variable names.', ...
                      badnames{1},badnames{2});
        else 
            badnamemsg = sprintf('%s"%s" and "%s" are not valid MATLAB variable names.', ...
                      sprintf('"%s", ',badnames{1:end-2}),badnames{end-1},badnames{end});
        end
    end
    
    if numbadentries > 0 
        dialogname = sprintf('Invalid variable names');
        if numbadentries == 1
            dialogname = sprintf('Invalid variable name');
        end
    ...
    if ~isempty(dupnames) 
        dialogname = sprintf('Duplicate variable names');
        if (length(dupnames) == 1)
            queststr = sprintf('"%s" already exists. Do you want to overwrite it?', ...
                dupnames{1});
            dialogname = sprintf('Duplicate variable name');
        elseif (length(dupnames) == 2)
            queststr = sprintf('"%s" and "%s" already exist. Do you want to overwrite them?', ...
                dupnames{1},dupnames{2});
        else
            queststr = sprintf('%s"%s" and "%s" already exist. Do you want to overwrite them?', ...
