function validateFirstDerivatives(funfcn,confcn,x,fval,cIneq,cEq,grad, ...
    JacCineqTrans,JacCeqTrans,lb,ub,fscale,options,sizes,varargin)
...
%   $Revision: 1.1.6.4.8.1 $  $Date: 2009/12/28 15:07:09 $
...
isGrad = true; 
scaleObjConstr = strcmpi(options.ScaleProblem,'obj-and-constr');

if strcmpi(options.GradObj,'on') 
    grad_fd = zeros(nVar,1); % input to finitedifferences()
    grad_fd = finitedifferences(x,funfcn{3},[],lb,ub,fval,[],[],1:nVar,isGrad, ...
        options,sizes,grad_fd,[],[],scaleObjConstr,fscale,varargin{:});
    % Unscale gradients
...
if strcmpi(options.GradConstr,'on') && sizes.mNonlinIneq + sizes.mNonlinEq > 0
    ...
    [unused,JacCineqTrans_fd,JacCeqTrans_fd] = finitedifferences(x,[],confcn{3}, ...
        lb,ub,[],-cIneq(:),cEq(:),1:nVar,isGrad,options,sizes,[],JacCineqTrans_fd, ...
        JacCeqTrans_fd,scaleObjConstr,fscale,varargin{:});
