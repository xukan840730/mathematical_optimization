function [alpha,f_alpha,grad,exitflag,funcCount] = ...
            lineSearch(funfcn,xInitial,numberOfVariables, ...
            dir,fInitial,fPrimeInitial,initialStepLength,rho,sigma, ...
            fminimum,maxFunEvals,TolFunLnSrch,finDiffOpts,sizes,grad,varargin)
...
%   $Revision: 1.1.6.9 $  $Date: 2008/12/01 07:20:48 $
...
[a,b,f_a,fPrime_a,f_b,fPrime_b,alpha,f_alpha,grad,exitflagBrckt,funcCountBrckt] = ...
   ...
   initialStepLength,rho,sigma,fminimum,maxFunEvals,finDiffOpts,sizes,grad,varargin{:});
...
if exitflagBrckt == 2 
  ...
  % within bracket
    [alpha,f_alpha,grad,exitflag,funcCount] = sectioningPhase(funfcn, ...
       ...
       finDiffOpts,sizes,grad,varargin{:}); 
else              
...
while funcCount < maxFunEvals 
  ...
  switch funfcn{1}
      case 'fun'
          ...
          [grad,unused1,unused2,numEvals] = finitedifferences(xInitial(:)+alpha*dir(:), ...
              funfcn{3},[],[],[],f_alpha,[],[],1:numberOfVariables,true,finDiffOpts,sizes, ...
              grad,[],[],[],[],varargin{:});
