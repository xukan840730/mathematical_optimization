...
%   $Revision.2 $  $Date: 2009/05/07 18:25:03 $
...
if (largescale)
    if (useSimplex)
        warning('optim:linprog:IgnoreSimplexOn', ...
                ['Simplex algorithm does not currently solve large-scale problems;\n' ...
                 'calling large-scale interior point algorithm. (To run simplex, set\n' ...
                 'OPTIONS.LargeScale to ''off''.)'])
    end
    if ~isempty(x0) && verbosity > 0
        warning('optim:linprog:IgnoreStartPoint', ...
                ['Large scale (interior point) algorithm uses a built-in starting point;\n' ...
...
elseif (useSimplex)
    if ~isempty(x0) && verbosity > 0
        warning('optim:linprog:IgnoreStartPoint', ...
               ['Simplex algorithm uses a built-in starting point; ignoring user-supplied X0.'])
    end
    % Set the default values of TolFun and MaxIter for this algorithm
    ...
    if ischar(lpmaxiter)
        if isequal(lower(lpmaxiter),'10*max(numberofvariables,numberofinequalities+numberofbounds)')
            ...
            error('optim:linprog:InvalidMaxIter', ...
