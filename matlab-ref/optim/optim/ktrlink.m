...
%   $Revision: 1.1.10.12 $  $Date: 2009/11/05 17:01:01 $
...
if strcmpi(computer,'maci64')
    error('optim:ktrlink:UnsupportedPlatform',...
        'ktrlink is unsupported for Mac OS X 64-bit.');
end
...
try
    ...
    optim_ME = MException('optim:ktrlink:ObjectiveError', ...
        'Failure in initial user-supplied objective function evaluation. ktrlink cannot continue.');
...
if constflag
    try
        ...
        if findstr(xlate('Too many output arguments'),userFcn_ME.message)
            ...
            optim_ME = MException('optim:ktrlink:NonlconError', ...
                'Failure in initial user-supplied nonlinear constraint function evaluation. ktrlink cannot continue.');
            userFcn_ME = addCause(userFcn_ME,optim_ME);
            rethrow(userFcn_ME)
        end
    end
...
if isempty(options.HessPattern)
    ...
    if (mHpattern ~= numberOfVariables) || (nHpattern ~= numberOfVariables)
        error('optim:ktrlink:InvalidHessPatternDims', ...
            'HessPattern expected to be of size %i-by-%i.',numberOfVariables,numberOfVariables);
    end
        ...
        if checkedGradEval
            ...
            try
                ...
                optim_ME = MException('optim:ktrlink:ObjGradError', ...
                    'Failure in user-supplied objective function gradient evaluation. ktrlink cannot continue.');
                userFcn_ME = addCause(userFcn_ME,optim_ME);
                rethrow(userFcn_ME)
            end
            if constflag            % If we have nonlinear constraints
                try
                    ...
                    optim_ME = MException('optim:ktrlink:ConstrGradError', ...
                        'Failure in user-supplied nonlinear constraint function gradient evaluation. ktrlink cannot continue.');
                    userFcn_ME = addCause(userFcn_ME,optim_ME);
                    rethrow(userFcn_ME)
                end
        ...
        if checkedHessEval
            ...
            try % Evaluating Hessian Function
                ...
                optim_ME = MException('optim:ktrlink:HessFcnError', ...
                    'Failure in user-supplied Hessian function evaluation. ktrlink cannot continue.');
                userFcn_ME = addCause(userFcn_ME,optim_ME);
                rethrow(userFcn_ME)
            end
        ...
        if checkedHessEval
            ...
            try % Evaluating Hessian-vector product
                ...
                optim_ME = MException('optim:ktrlink:HessMultError', ...
                    'Failure in user-supplied HessMult function evaluation. ktrlink cannot continue.');
                userFcn_ME = addCause(userFcn_ME,optim_ME);
                rethrow(userFcn_ME)
            end
            checkedHessEval = true;
...
function [indfun,indvar,nnzj,JacIdx,subIndexJac,Jrow,Jcol] = getJacIdx(havePattern,...
    JacStrct,numConstr,numVars,indfun,indvar,nnzj,nnzCon,maxfunind)
